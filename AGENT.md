...
| `/try` | A chat-styled demo that makes **real, billed** calls on the gateway's own credit. Bounded by `McpGateway.Demo`: only fixed examples run, and the demo account is topped up once per UTC day so its spend is capped however much traffic arrives. |
